...
void EnableRtcIntPin()
{
	uint8_t ctr3[2] =   {SD30XX_REG_CTR3, 0x80};
    uint8_t ctr2[2] =   {SD30XX_REG_CTR2, 0x92};
	uint8_t buffer[8]=	{SD30XX_REG_ALARM_SEC,0,0,0,0,0,0,1};	

    writeEnable();
	i2c_write_blocking(I2C_PORT, SD30XX_ADDR, ctr3, 2, false); 
    writeDisable();
	writeEnable();
	i2c_write_blocking(I2C_PORT, SD30XX_ADDR, ctr2, 2, false); 
    writeDisable();
	i2c_write_blocking(I2C_PORT, SD30XX_ADDR, buffer, 8, false); 	
}
...
void DisableRtcIntPin()
{
    uint8_t rdat[1]={0};
    uint8_t wdat[1]={0};
    wdat[0]=SD30XX_REG_CTR1;
	i2c_write_blocking(I2C_PORT, SD30XX_ADDR, wdat, 1, true);
    sleep_ms(10); 
	i2c_read_blocking(I2C_PORT, SD30XX_ADDR, rdat, 1, false);
}

void EnableRtcI2CInVbat()
{
    uint8_t en[2] =   {SD30XX_REG_AGTC, 0x00};//0x00};//0x80};
	writeEnable();
    i2c_write_blocking(I2C_PORT, SD30XX_ADDR, en, 2, false);
	writeDisable();
}
